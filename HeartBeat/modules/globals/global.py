...
from HeartBeat.database import gbandb as HeartBeat
from HeartBeat.database import gmutedb as Gmute
...
async def gban_user(client: Client, message: Message):
    ...
    if (await HeartBeat.gban_info(user.id)):
    ...
    for gokid in f_chats:
        try:
            ...
            er += 1
    await HeartBeat.gban_user(user.id)
    ok.append(user.id)
    msg = (
        r"**\\#GBanned_User//**"
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await HeartBeat.gban_info(user.id)):
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await HeartBeat.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
...
async def gbanlist(client: Client, message: Message):
    users = (await HeartBeat.gban_list())
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await HeartBeat.gban_info(user_id)):
        try:
